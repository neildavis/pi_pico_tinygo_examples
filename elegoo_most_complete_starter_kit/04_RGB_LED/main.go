...
func main() {
	...
	for {
		...
		for j := 0; j < 3; j++ {
			...
			for i := 0; i < 255; i++ {
				pwmRed.Set(chanRed, pwmRed.Top()*uint32(redValue)/255)
				pwmGreen.Set(chanGreen, pwmGreen.Top()*uint32(greenValue)/255)
				pwmBlue.Set(chanBlue, pwmBlue.Top()*uint32(blueValue)/255)
				*incVal++
				*decVal--
