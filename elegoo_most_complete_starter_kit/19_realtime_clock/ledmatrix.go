...
// Only the chars we have defined are valid
func displayChar(ch byte) {
	clearLED()
	charBytes := chars[ch]
	for row, val := range charBytes {
		setLEDRow(row, val)
	}
}

// Only the chars we have defined are valid in text
